...
```
## 중단하기 (break)
데이터를 순회하던 중, 반복문을 중단하지 않고 다음 반복으로 건너뛸 때 사용하다.
```python
...
## break확인 (else)
for문으로 데이터를 순회하던 중, break문이 호출되지 않고 반복문이 종료되며 else문이 실행된다.
```python
for 항목 in iterable객체:
  pass
else:
...
```
zip으로 반환되는 것은 리스트가 아니 zip클래스 형태의 iterable객체이기 때문에, 리스트 형태로 사용하려면 list()함수를 사용해준다.<br>
